...
# https://kminito.tistory.com/36  # 조건검색 후 텔레그램 메시지 수신하기
...
class Kiwoom(QAxWidget):
    ...
    def trdata_slot(self):
        data = self.dynamicCall('GetCommDataEx(Qstring, Qstring)', 'opt10081', '주식일봉차트조회')
        for i in range(3):
            print('날짜', data[i][4])
            print('현재가: ', data[i][1], '거래량: ', data[i][2], '거래대금: ', data[i][3])
            print('시가: ', data[i][5], '고가: ', data[i][6], '저가: ', data[i][7])
    ...
    def day_kiwoom_db(self, code):
        ...
        self.dynamicCall('CommRqData(QString, QString, int, QString)', '주식일봉차트조회', 'opt10081', '0', '4000')
